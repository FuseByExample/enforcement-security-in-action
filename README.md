...
- {httpie} or {curl}
- http://www.apiman.io/latest/download.html[Apiman 1.2.1.Final]
- JDK 1.7 or 1.8
- Apache Maven 3.2.3

== Installation

In this demo, ApiMan & Keycloak are used to secure the Camel REST endpoints created within the REST DSL in Action demo.
...
+
. Click on the `Services` Tab of the `Fuse` organization to register the first REST Service to be secured.
+
...
. In the `Services` screen of the `Fuse` organization, click the button: `New Service`.
